/*
 * HNSW V2 — 在 V1 基础上做了两处修改：
 * 1. search_knn 增加 ef_search 参数，召回率现在可以通过这个参数调节
 * 2. search_layer 里加了流水线预取，遍历邻居时提前把下一批数据拉进缓存
 */
...
public:
    ...
    // ef_search 默认 0 时行为和 V1 一样，传入具体值可以控制召回率
